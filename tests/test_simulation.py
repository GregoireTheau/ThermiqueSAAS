...
def test_heatwave_after_reflective_roof_simulation():
    ...
    assert round(results["rooms_summary"]["living_room"]["cooling_thermal_kwh"], 2) == 33.88
    assert round(results["rooms_summary"]["bedroom"]["hot_degree_hours"], 1) == 128.3
    assert round(results["rooms_summary"]["bedroom"]["solar_gain_kwh"], 2) == 5.13
    assert round(results["rooms_summary"]["bedroom"]["transmission_exchange_kwh"], 2) == 0.63
    assert round(results["rooms_summary"]["bedroom"]["ventilation_exchange_kwh"], 2) == 0.11
    bedroom_max = max(
